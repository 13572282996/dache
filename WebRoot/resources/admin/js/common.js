...
var setting = {
	...
	uploadFlashExtension: "swf,flv,mp4",
...
(function($) {
	...
	$.dialog2 = function(options) {
		...
		var $dialog = $('<div id="xxDialogClose" class="xxDialog"><\/div>');
		var $dialogTitle;
		var $dialogClose = $('<div class="dialogClose"><\/div>').appendTo($dialog);
		var $dialogContent;
		var $dialogBottom;
		...
		$dialog.appendTo("body");
		if (settings.modal) {
			$dialogOverlay = $('<div id="dialogOverlayClose" class="dialogOverlay"><\/div>').insertAfter($dialog);
		}
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: web.base + "/admin/file/browser.jhtml",
				uploadUrl: web.base + "/admin/file/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = web.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = web.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
