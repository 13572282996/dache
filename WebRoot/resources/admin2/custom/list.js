		...
		var $checkedIds = $("#listTable input[name='ids']:enabled:checked");
				$.ajax({
					...
					success: function(data) {
						...
						$.dialog({
							type: "warn",
							content:'<form id="paymentForm" action="xiazai.jhtml" method="get"><input type="hidden" id="filePath" name="filePath" value="" \/>excel生成成功点击确定下载<\/form>',
							ok: message("admin.dialog.ok"),
							cancel: message("admin.dialog.cancel"),
							onOk: function() {
								$("#filePath").val(data.filePath2);
								$("#paymentForm").submit();
								return true;
							}
						});
