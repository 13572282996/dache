...
-->      
        <ul class="breadcrumb">
            ...
            <li class="active">文档列表</li> (${message("admin.page.total", page.total)})
        </ul>
...
<!--指定按名称搜索	<select name="searchProperty" class="listselect">
	...
	<input type="hidden" name="searchProperty" value="fileName" />
...
<div class="well" style="margin-bottom: 0px;padding-bottom: 0px;">
    <table class="table" id="listTable">
     <tr>
							...
							<th>
								<a href="javascript:;" name="">所属分类</a>
							</th>
							<th>
								<a href="javascript:;"  name="">文件名</a>
							</th>
							<th>
								<span>文件保存路径</span>
							</th>
							<th>
								<a href="javascript:;"  name="createDate">${message("admin.common.createDate")}</a>
							</th>
							<th>
								<span>${message("admin.common.handle")}</span>
							</th>
						</tr>
						[#list page.content as reference]
							<tr>
								<td><input type="checkbox" name="ids" value="${reference.id}" /></td>
								<td>
								${reference.articleCategory.name}
								</td>
								<td>${reference.fileName}</td>
								<td>${reference.filePath}</td>
								<td>
									<span title="${reference.createDate?string("yyyy-MM-dd HH:mm:ss")}">${reference.createDate}</span>
								</td>
								<td>
									<a href="edit.jhtml?id=${reference.id}">[${message("admin.common.edit")}]</a>
								</td>
							</tr>
						[/#list]
